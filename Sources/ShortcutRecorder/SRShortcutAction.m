...
- (BOOL)performActionOnTarget:(id)aTarget
{
    ...
    os_activity_initiate("-[SRShortcutAction performActionOnTarget:]", OS_ACTIVITY_FLAG_DEFAULT, ^{
        if (!self.isEnabled)
        {
            os_log_debug(SR_LOG_DEFAULT, "Not performed: disabled");
            return;
        }
        ...
        if (actionHandler)
        {
            os_log_debug(SR_LOG_DEFAULT, "Using action handler");
        ...
        else
        {
            ...
            if (!target)
            {
                os_log_debug(SR_LOG_DEFAULT, "Not performed: no associated target");
                return;
            }
            ...
            {
                os_log_debug(SR_LOG_DEFAULT, "Not performed: target cannot respond to action");
                return;
            }
            else if ([target respondsToSelector:@selector(validateUserInterfaceItem:)] && ![target validateUserInterfaceItem:self])
            {
                os_log_debug(SR_LOG_DEFAULT, "Not performed: target ignored action");
                return;
            }

            if (canPerformAction)
            {
                os_log_debug(SR_LOG_DEFAULT, "Using action");
            ...
            {
                os_log_debug(SR_LOG_DEFAULT, "Using protocol");
    ...
    switch (anEventType)
    {
        ...
        case NSEventTypeFlagsChanged:
        {
            ...
            if (keyCode == kVK_Command || keyCode == kVK_RightCommand)
                ...
                os_log_error(SR_LOG_DEFAULT, "#Error Unexpected key code %hu for the FlagsChanged event", keyCode);
            break;
        }
        default:
...
static OSStatus _SRCarbonEventHandler(EventHandlerCallRef aHandler, EventRef anEvent, void *aUserData)
{
    if (!anEvent)
    {
        os_log_error(SR_LOG_DEFAULT, "#Error Event is NULL");
        return eventNotHandledErr;
    }
    else if (GetEventClass(anEvent) != kEventClassKeyboard)
    {
        os_log_error(SR_LOG_DEFAULT, "#Error Not a keyboard event");
        return eventNotHandledErr;
    }
    else
...
- (void)resume
{
    @synchronized (_actions)
    {
        os_log_debug(SR_LOG_DEFAULT, "Global Shortcut Monitor counter: %ld -> %ld", _disableCounter, _disableCounter - 1);
        _disableCounter -= 1;
...
- (OSStatus)handleEvent:(EventRef)anEvent
{
    ...
    os_activity_initiate("-[SRGlobalShortcutMonitor handleEvent:]", OS_ACTIVITY_FLAG_DETACHED, ^{
        ...
        {
            os_log_debug(SR_LOG_DEFAULT, "Monitoring is currently disabled");
            return;
        }
        ...
        {
            os_log_error(SR_LOG_DEFAULT, "#Critical Failed to get hot key ID: %d", error);
            return;
        }
        else if (hotKeyID.id == 0 || hotKeyID.signature != SRShortcutActionSignature)
        {
            os_log_error(SR_LOG_DEFAULT, "#Error Unexpected hot key with id %u and signature: %u", hotKeyID.id, hotKeyID.signature);
            return;
        }
        ...
        @synchronized (self->_actions)
        {
            ...
            if (!shortcut)
            {
                os_log_info(SR_LOG_DEFAULT, "Unregistered hot key with id %u and signature %u", hotKeyID.id, hotKeyID.signature);
                return;
            }
            ...
            switch (GetEventKind(anEvent))
            {
                ...
                case kEventHotKeyReleased:
                    ...
                    break;
                default:
                    os_log_error(SR_LOG_DEFAULT, "#Error Unexpected key event of type %u", GetEventKind(anEvent));
                    return;
            }
            ...
            if (!actions.count)
            {
                os_log_info(SR_LOG_DEFAULT, "No actions for the shortcut");
                return;
            }
...
{
    os_log_debug(SR_LOG_DEFAULT, "Added Carbon HotKey Event Handler");
}

- (void)didRemoveEventHandler
{
    os_log_debug(SR_LOG_DEFAULT, "Removed Carbon HotKey Event Handler");
}
...
- (void)_installEventHandlerIfNeeded
{
    ...
    os_log_info(SR_LOG_DEFAULT, "Installing Carbon hot key event handler");
    ...
    if (error != noErr)
    {
        os_log_error(SR_LOG_DEFAULT, "#Critical Failed to install event handler: %d", error);
...
- (void)_removeEventHandlerIfNeeded
{
    ...
    os_log_info(SR_LOG_DEFAULT, "Removing Carbon hot key event handler");
    OSStatus error = RemoveEventHandler(_carbonEventHandler);

    if (error != noErr)
        os_log_error(SR_LOG_DEFAULT, "#Error Failed to remove event handler: %d", error);
...
- (void)_registerHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    {
        os_log_error(SR_LOG_DEFAULT, "#Error Shortcut without a key code cannot be registered as Carbon hot key");
        return;
    }

    static UInt32 CarbonID = _SRInvalidHotKeyID;
    EventHotKeyID hotKeyID = {SRShortcutActionSignature, ++CarbonID};
    os_log_info(SR_LOG_DEFAULT, "Registering Carbon hot key");
    ...
    if (error != noErr || !hotKey)
    {
        ...
        os_log_error(SR_LOG_DEFAULT, "#Critical Failed to register Carbon hot key: %d, keyCode: %d, modifierFlags: %lu", error, aShortcut.keyCode, (unsigned long)aShortcut.modifierFlags);
        return;
    }

    os_log_info(SR_LOG_DEFAULT, "Registered Carbon hot key %u, keyCode: %d, modifierFlags: %lu", hotKeyID.id, aShortcut.keyCode, (unsigned long)aShortcut.modifierFlags);
...
- (void)_unregisterHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    os_log_info(SR_LOG_DEFAULT, "Removing Carbon hot key %u", hotKeyID);
    OSStatus error = UnregisterEventHotKey(hotKey);

    if (error != noErr)
    {
        os_log_error(SR_LOG_DEFAULT, "#Critical Failed to unregister Carbon hot key %u: %d, keyCode: %hu, modifierFlags: %lu", hotKeyID, error, aShortcut.keyCode, aShortcut.modifierFlags);
    }
    else
    {
        os_log_error(SR_LOG_DEFAULT, "Unregistered Carbon hot key %u, keyCode: %hu, modifierFlags: %lu", hotKeyID, aShortcut.keyCode, aShortcut.modifierFlags);
    }
...
CGEventRef _Nullable _SRQuartzEventHandler(CGEventTapProxy aProxy, CGEventType aType, CGEventRef anEvent, void * _Nullable aUserInfo)
{
    ...
    {
        os_log_error(SR_LOG_DEFAULT, "#Error #Developer The system disabled event tap due to %u", aType);
        CGEventTapEnable(self.eventTap, true);
        return anEvent;
    }
    else if (aType != kCGEventKeyDown && aType != kCGEventKeyUp && aType != kCGEventFlagsChanged)
    {
        os_log_error(SR_LOG_DEFAULT, "#Error #Developer Unexpected event of type %u", aType);
        return anEvent;
    }
    else
...
- (instancetype)initWithRunLoop:(NSRunLoop *)aRunLoop tapOptions:(CGEventTapOptions)aTapOptions
{
    ...
    if (!eventTap)
    {
        os_log_error(SR_LOG_DEFAULT, "#Critical Unable to create event tap: make sure Accessibility is enabled");
        return nil;
    }
...
- (CGEventRef _Nullable)handleEvent:(CGEventRef)anEvent
{
    __block _Nullable CGEventRef result = anEvent;
    ...
    if (!result && !_canActivelyFilterEvents)
        os_log_error(SR_LOG_DEFAULT, "#Developer #Error The monitor is not configured to actively filter events");
...
+ (NSDictionary<NSString *, id> *)_parseSystemKeyBindings
{
    ...
    if (@available(macOS 10.13, *))
    {
        ...
        {
            os_log_error(SR_LOG_DEFAULT, "#Error unable to read system key bindings: %s", error.localizedDescription.UTF8String);
            systemKeyBindings = @{};
        }
    }
    else
    {
        ...
        if (!systemKeyBindings)
        {
            os_log_error(SR_LOG_DEFAULT, "#Error unable to read system key bindings");
            systemKeyBindings = @{};
        }
    }
...
+ (NSDictionary<NSString *, id> *)_parseUserKeyBindings
{
    ...
    if (@available(macOS 10.13, *))
    {
        ...
        if (!userKeyBindings)
        {
            os_log_debug(SR_LOG_DEFAULT, "#Error unable to read user key bindings: %s", error.localizedDescription.UTF8String);
            userKeyBindings = @{};
        }
    }
    else
    {
        ...
        if (!userKeyBindings)
        {
            os_log_debug(SR_LOG_DEFAULT, "#Error unable to read user key bindings");
            userKeyBindings = @{};
        }
    }
