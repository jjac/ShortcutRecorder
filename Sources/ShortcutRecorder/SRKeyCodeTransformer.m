                             ...
                             usingCache:(BOOL)anIsUsingCache
{
    ...
    if (!inputSource)
    {
        os_log_error(SR_LOG_DEFAULT, "#Critical Failed to create an input source");
        return nil;
    }
    ...
    if (anIsUsingCache)
    {
        ...
        if (sourceIdentifier)
        {
            ...
            os_log_error(SR_LOG_DEFAULT, "#Error Input source misses an ID");
    }
    ...
    @synchronized (self)
    {
        ...
        if (cacheKey)
        {
            ...
            if (translation)
            {
                os_log_debug(SR_LOG_DEFAULT, "Translation cache hit");
                return translation;
            }
            else
                os_log_debug(SR_LOG_DEFAULT, "Translation cache miss");
        }
        ...
        if (error != noErr)
        {
            os_log_error(SR_LOG_DEFAULT, "#Error Unable to translate keyCode %hu and modifierFlags %lu: %d",
                           aKeyCode,
        ...
        {
            os_log_debug(SR_LOG_DEFAULT, "#Error No translation exists for keyCode %hu and modifierFlags %lu",
                           aKeyCode,
                           anImplicitModifierFlags);
            return nil;
        }
...
- (NSNumber *)keyCodeForTranslation:(NSString *)aTranslation
{
    ...
    @synchronized (self)
    {
        ...
        os_log_debug(SR_LOG_DEFAULT, "Updating translation -> key code mapping");
...
- (instancetype)init
{
    ...
    {
        os_log_error(SR_LOG_DEFAULT, "#Developer #Error Use SRSymbolicKeyCodeTransformer instead");
               ...
               layoutDirection:(NSUserInterfaceLayoutDirection)aDirection
{
    ...
    os_activity_initiate("Key Code -> Literal", OS_ACTIVITY_FLAG_DEFAULT, ^{
        ...
        {
            os_log_error(SR_LOG_DEFAULT, "#Error Invalid key code");
            return;
        }
...
- (NSNumber *)reverseTransformedValue:(NSString *)aValue
{
    ...
    os_activity_initiate("ASCII Literal -> Key Code", OS_ACTIVITY_FLAG_DEFAULT, ^{
        ...
        {
            os_log_error(SR_LOG_DEFAULT, "#Error Invalid ASCII literal");
            return;
        }
    ...
    if (!result)
    {
        os_log_error(SR_LOG_DEFAULT, "#Error Invalid value for reverse transformation");
    }
...
- (NSNumber *)reverseTransformedValue:(NSString *)aValue
{
    ...
    os_activity_initiate("ASCII Symbol -> Key Code", OS_ACTIVITY_FLAG_DEFAULT, ^{
        ...
        {
            os_log_error(SR_LOG_DEFAULT, "#Error Invalid ASCII symbol");
            return;
        }
