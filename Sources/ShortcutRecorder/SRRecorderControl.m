...
- (void)drawBackground:(NSRect)aDirtyRect
{
    ...
    {
        os_log_debug(SR_LOG_DEFAULT, "#Developer drawing background using color");
...
- (void)propagateValue:(id)aValue forBinding:(NSString *)aBinding
{
    ...
    {
        os_log_error(SR_LOG_DEFAULT, "#Error The control is bound to NSUserDefaults but is not transformed into an allowed CFPreferences value");
        NSLog(@"WARNING: Shortcut Recroder 2 compatibility mode enabled. Getters of objectValue and NSValueBinding will return an instance of NSDictionary.");
        _isCompatibilityModeEnabled = YES;
...
- (BOOL)canCaptureKeyEvent
{
    if (!self.enabled)
    {
        os_log_debug(SR_LOG_DEFAULT, "The control is disabled");
        return NO;
    }
    else if (self.window.firstResponder != self)
    {
        os_log_debug(SR_LOG_DEFAULT, "The control is not the first responder");
        return NO;
    }
    else if (self->_mouseTrackingButtonTag != _SRRecorderControlInvalidButtonTag)
    {
        os_log_debug(SR_LOG_DEFAULT, "The control is tracking %lu", self->_mouseTrackingButtonTag);
        return NO;
    }
    else
        return YES;
...
#pragma clang diagnostic pop
    ...
    os_activity_initiate("-[SRRecorderControl canEndRecordingWithObjectValue:]", OS_ACTIVITY_FLAG_DEFAULT, ^{
        if ([self areModifierFlagsValid:aShortcut.modifierFlags forKeyCode:aShortcut.keyCode])
        {
            ...
            {
                os_log_debug(SR_LOG_DEFAULT, "Valid and accepted shortcut");
                result = YES;
            }
            else
            {
                os_log_debug(SR_LOG_DEFAULT, "Delegate rejected");
                result = NO;
            }
        }
        else
        {
            os_log_debug(SR_LOG_DEFAULT, "Modifier flags %lu rejected", aShortcut.modifierFlags);
            result = NO;
        }
    });
    ...
    os_activity_initiate("-[SRRecorderControl performKeyEquivalent:]", OS_ACTIVITY_FLAG_DEFAULT, ^{
        ...
        if (self.isRecording)
        {
            if (anEvent.keyCode == USHRT_MAX)
            {
                ...
                os_log_debug(SR_LOG_DEFAULT, "Invalid key code");
                result = NO;
            }
            ...
            {
                os_log_debug(SR_LOG_DEFAULT, "Cancel via Esc");
                [self endRecording];
                result = YES;
            }
                    ...
                    (anEvent.modifierFlags & SRCocoaModifierFlagsMask) == 0)
            {
                os_log_debug(SR_LOG_DEFAULT, "Clear via Delete");
                [self clearAndEndRecording];
                result = YES;
            }
            else
        ...
        {
            os_log_debug(SR_LOG_DEFAULT, "Begin recording via Space");
